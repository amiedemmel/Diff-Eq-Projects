%HW 8 Problem 3.1.4 part a
%Amie Demmel
%box1.m
%
%This script impliments scheme 3.1.23 box method for ut+a*ux=f where f is
%defined in f.m
...
while(flag)
    ...
    for j=1:n-1
        ...
        for i=1:m-1
            %Box Scheme 3.1.23
            boxf=f(tt,x+h)+f(tt,x)+f(t,x+h)+f(t,x);
            u(j+1,i+1)=(1+a*lb)*u(j,i)+(1-a*lb)*u(j,i+1)-(1-a*lb)*u(j+1,i)+k/2*boxf;
